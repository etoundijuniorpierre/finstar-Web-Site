...
export type SeoPageKey = 'HOME' | 'ABOUT' | 'SERVICES' | 'CAREER' | 'CONTACTS' | 'FAQ' | 'METRICS';
...
export class SeoService {
    ...
    updatePageSeo(pageKey: SeoPageKey): void {
        ...
        const hasValidTitle = typeof title === 'string' && title.trim().length > 0 && title !== `${baseKey}.TITLE`;
        const hasValidDescription = typeof description === 'string' && description.trim().length > 0 && description !== `${baseKey}.DESCRIPTION`;
    ...
    private ensureAbsoluteUrl(url: string): string {
        if (!url) return url;
        return url.startsWith('http') ? url : new URL(url, this.siteOrigin).toString();
    }

    async updateSeoAsync(config: SeoConfig & { useDynamicImage?: boolean }): Promise<void> {
    ...
    private updateAlternateLinks(): void {
        ...
        for (const lang of this.i18nService.supportedLanguages) {
            try {
                const href = new URL(`/${lang}${normalizedPath}`, this.siteOrigin).toString();
        ...
        try {
            ...
            defaultLink.href = new URL(`/fr-FR${normalizedPath}`, this.siteOrigin).toString();
