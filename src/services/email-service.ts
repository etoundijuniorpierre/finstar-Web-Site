...
export class EmailService {
  ...
  async sendEmail(formData: ContactFormData): Promise<boolean> {
    ...
    const targetEmail = formData.isCareer
      ? emailConfig.careerEmail
    ...
    try {
          ...
          .pipe(
            ...
            retry({
              ...
              delay: (error, retryCount) => {
                console.log(
                  `Nouvelle tentative ${retryCount + 1}/${this.maxRetries + 1}...`,
                );
      ...
      if (isSuccess) {
        ...
        throw new Error(
          `Réponse inattendue d'EmailJS: ${response.body} (Status: ${response.status})`,
        );
      }
    } catch (error: any) {
      ...
      if (error.status === 422) {
        ...
        errorMessage =
          'Le template EmailJS ne reconnaît pas les variables de destination.';
        console.error('❌ Erreur 422 : "The recipients address is empty"');
        console.error(
          'CONSEIL : Vérifiez que dans votre Dashboard EmailJS, le champ "To Email" du template contient exactement {{to_email}}.',
        );
      }
...
Veuillez cliquer sur les liens ci-dessus pour ouvrir les documents.
  ...
  private validateFormData(formData: ContactFormData): boolean {
    ...
    const validations = [
      {
        field: formData.name,
        condition: !formData.name || formData.name.trim().length < 2,
        message: 'Le nom doit contenir au moins 2 caractères',
      },
      {
        field: formData.email,
        condition:
          (!formData.email || !this.isValidEmail(formData.email)) &&
          (!formData.phoneNumber || formData.phoneNumber.trim().length < 9),
        message:
          'Veuillez saisir une adresse email valide ou un numéro de téléphone',
      },
      {
        field: formData.subject,
        condition: !formData.subject || formData.subject.trim().length < 3,
        message: 'Le sujet doit contenir au moins 3 caractères',
      },
      {
        field: formData.message,
        condition: !formData.message || formData.message.trim().length < 5,
  ...
  async diagnoseEmailJSConfiguration(): Promise<void> {
    ...
    const diagnosis = {
      publicKey: config.publicKey
        ? `✅ Défini (${config.publicKey.substring(0, 8)}...)`
        : '❌ Manquant',
      serviceId: config.serviceId
        ? `✅ Défini (${config.serviceId})`
        : '❌ Manquant',
      templateIdEmail: config.templateIdEmail
        ? `✅ Défini (${config.templateIdEmail})`
        : '❌ Manquant',
      contactEmail: config.contactEmail
        ? `✅ ${config.contactEmail}`
        : '❌ MANQUANT - ERREUR CRITIQUE',
    ...
    try {
      const testResponse = await fetch('https://api.emailjs.com', {
        method: 'GET',
        mode: 'no-cors',
      });
